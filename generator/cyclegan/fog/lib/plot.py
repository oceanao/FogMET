...
def clear2fog_intensity(model_clear2fog, image_clear, intensity=0.5,
                             normalized_input=True, close_fig=False):
    import matplotlib.pyplot as plt
    import tensorflow as tf
    # print("----------------------------ok[[[[[[[[[[[[[[[[[[[[[[[[")
    ...
    intensity = tf.expand_dims(tf.expand_dims(tf.convert_to_tensor(intensity), 0), 0)
    prediction_clear2fog = model_clear2fog((tf.expand_dims(image_clear, 0), intensity))
    if normalized_input: #序列化
        to_display = prediction_clear2fog[0] * 0.5 + 0.5
    #print(to_display)
    import  cv2
    # cv2.imshow("8",to_display.numpy())
    # cv2.waitKey()
    # print(to_display.numpy())
    # cv2.imwrite("test.png",to_display.numpy()*255)
    z_np = to_display.numpy()
    # print(z_np*255)
    return z_np
